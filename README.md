...
1. create an environment `cricsheet` with the help of [conda],
   ```
   conda env create -f environment.yaml
   ```
2. activate the new environment with
   ```
   conda activate cricsheet
   ```
3. install `cricsheet` with:
   ```
...
2. Create concrete dependencies as `environment.lock.yaml` for the exact reproduction of your
   environment with:
   ```
   conda env export -n cricsheet -f environment.lock.yaml
   ```
...
├── src
│   └── cricsheet           <- Actual Python package where the main functionality goes.
