...
class MatchParser(BaseParser):
    ...
    def parse_match_info(self, soup):
        ...
        match_info_data = np.array(
            [item.text.strip() for item in soup.find_all(class_="TextBlack8")]
        )[:5]
        log.debug("Parsing Match Info: category")
        match_info_category = np.array(
            [item.text.strip() for item in soup.find_all(class_="TextBlackBold8")]
        )[:5]

        log.debug("Parsing Match Info: formatting")
        d_match_info = pd.DataFrame(match_info_data, match_info_category).T.to_dict(
            orient="records"
        )[0]
        d_match_info["Match Date:"] = parse(
            d_match_info["Match Date:"], fuzzy=True
        ).date()
    ...
    def parse_single_scorecard(self, innings_section):
        ...
        l_headers = [
            x.text.replace("\xa0", " ").strip()
            for x in innings_section.parent.find_all("td")
        ]
        ...
        while True:
            ...
            next_node = (
                next_node.find_next_sibling()
            )  # use find_next_sibling() instead of next_sibling to avoid line breaks
            row = [
                x.text.replace("\xa0", " ").replace("\r", "").replace("\n", "").strip()
                for x in next_node.find_all("td")
            ]
    ...
    def clean_single_scorecard(
        self, df_scorecard, team_innings, match_innings, d_match_info
    ):
        """
        ...
        # Clean up data types
        df["MatchDate"] = df["MatchDate"].apply(
            pd.to_datetime, errors="coerce", yearfirst=True
        )
        ...
        df.loc[df.Player == "Total", df.columns.isin(["BF", "4s", "6s"])] = list(
            df.iloc[:-1, :].agg(aggs)
        )

        # Calculate innings strike rate
        df.loc[df.Player == "Total", "SR"] = (
            df[df.Player == "Total"].R / df[df.Player == "Total"].BF * 100
        )
    ...
    def parse_all_scorecards(self, soup, match_info):
        ...
        for item in soup.find_all(class_="TextBlackBold8"):
            ...
            if "Innings" in item_text:
                ...
                df_scorecard_clean = self.clean_single_scorecard(
                    df_scorecard, team_innings, match_innings, match_info
                )
    ...
    def parse_single_fall_of_wickets(self, fow_section):
        ...
        for x in td:
            d_row = {}
            row = x.text.strip().replace("\xa0", " ")
            log.debug(row)
            row = row.split(
                "  "
            )  # split in double space, which separates the fall of wicket from the player name
            log.debug(row)
            log.debug(row[-1])
            score = row[0].split("-")
    ...
    def clean_single_fall_of_wickets(
        self, df_fow, team_innings, match_innings, d_match_info
    ):
        """
    ...
    def parse_all_fall_of_wickets(self, soup, l_innings):
        ...
        for item in fow_sections:
            ...
            if len(df_fow) > 0:
                try:
                    df_fow_clean = self.clean_single_fall_of_wickets(
                        df_fow, team_innings, match_innings, self.match_info
                    )
    ...
    def execute(self):
        ...
        self.scorecards, l_innings = self.parse_all_scorecards(
            self.soup, self.match_info
        )  # l_innings required for FoW parsing, but not to keep as an attribute
        log.debug("Parsing Scorecards...Complete!")

        log.debug("Parsing Fall of Wickets...")
        self.fall_of_wickets = self.parse_all_fall_of_wickets(
            self.soup, l_innings
        )  # l_innings req., so always execute scorecard parsing first
