...
class Match(Base):
    ...
    def __repr__(self):
        return (
            "<Match(id='%s', venue='%s', city='%s', match_type='%s', start_date='%s')>"
...
class Scoresheet(Base):
    ...
    def __repr__(self):
        return (
            "<Scoresheet(match_id='%s', data_version='%s', created='%s', revision='%s')>"
...
class Wicket(Base):
    ...
    delivery = relationship(
        "Delivery", foreign_keys=[match_id, innings_number, over_number, ball_number]
    )
    ...
    def __repr__(self):
        return "<Wicket(player_out='%s', kind='%s')>" % (self.batsman_name, self.kind)
