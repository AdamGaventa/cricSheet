...
class Match:
    ...
    def parse_metadata(self, data):
        ...
        d_meta["date_start"] = meta["dates"][
            0
        ]  # first entry in dates field - first day of match if Test, match date otherwise
        ...
        if "winner" in meta["outcome"]:
            ...
            d_meta["outcome_by_type"], d_meta["outcome_by_value"] = next(
                iter(meta["outcome"]["by"].items())
            )
        else:
