...
#pd.read_csv(os.getcwd() + '/data/raw/csv/howstat/fall_of_wickets/fow_1.csv')
  ...
  def preprocess_files(self, l_fow_paths, l_score_paths):
    ...
    for i in range(len(l_fow_paths)):
      ...
      df_scores = pd.read_csv(l_score_paths[i], index_col=0, parse_dates=[2], infer_datetime_format=True)
  ...
  def execute(self):
    ...
    l_fow_paths, l_score_paths = self.get_filenames(self.filepath_fow, self.filepath_scores)

    log.info('Preprocessing data')
    df_processed_fow = self.preprocess_files(l_fow_paths, l_score_paths)

    log.info('Extracting collapses')
    df_collapses = df_processed_fow.groupby(['MatchId', 'MatchDate', 'MatchInnings', 'Team']).apply(return_collapses).reset_index()
